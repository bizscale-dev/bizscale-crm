...
import { updateCompletedLinksSheetUrl } from '../settings/actions';

export default function SyncLinksClient() {
  ...
  // Fetch saved URL on component mount
  useEffect(() => {
    fetchSavedUrl();
  }, []);
  ...
  const fetchSavedUrl = async () => {
    try {
      ...
      if (data.completedLinksSheetUrl && data.completedLinksSheetUrl !== 'NOT SET') {
        setSavedUrl(data.completedLinksSheetUrl);
      } else if (data.googleSheetsUrl && data.googleSheetsUrl !== 'NOT SET') {
        // Backward compatibility: fall back to the general sheet URL if no dedicated one is saved yet
        setSavedUrl(data.googleSheetsUrl);
      } else {
        setSavedUrl('');
      }
    } catch (err) {
  ...
  const handleSaveUrl = async () => {
    ...
    try {
      ...
      const result = await updateCompletedLinksSheetUrl(null, formData);
  ...
  const handleSync = async () => {
    ...
    try {
      // Use custom URL if provided, otherwise use saved URL, otherwise use nothing (fallback to settings)
      const urlToUse = sheetUrl || savedUrl || undefined;

      const res = await fetch('/api/sync-completed-links', {
        method: 'POST',
  ...
  return (
    <div className="card">
      <div style={{ marginBottom: '1.5rem' }}>
        <h2 style={{ fontSize: '1.25rem', marginBottom: '1rem', margin: 0, marginBottom: '1rem' }}>
          Configure & Sync
        </h2>
        ...
        <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: 'var(--background)', borderRadius: '0.5rem', border: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
            <label style={{ fontWeight: '600', fontSize: '0.95rem' }}>Google Sheet URL for Sync</label>
            <button
              type="button"
          ...
          <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            &quot;Save as Default&quot; persists this URL for the daily 5 PM, 7 PM, 9 PM &amp; 11:50 PM (PKT) sync and future visits to this page.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          ...
          >
            {loading ? 'Fetching...' : '🔄 Fetch & Update Now'}
          </button>
      ...
      {syncResult && (
        <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border)', paddingTop: '1.5rem' }}>
          <h3 style={{ fontSize: '1rem', marginBottom: '1rem', margin: 0, marginBottom: '1rem' }}>
            Sync Summary
          </h3>
          ...
          {syncResult.syncedClients && syncResult.syncedClients.length > 0 && (
            <div>
              <h4 style={{ fontSize: '0.9rem', marginBottom: '0.75rem', margin: 0, marginBottom: '0.75rem', color: 'var(--text-muted)' }}>
                Clients Synced
              </h4>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                  <thead style={{ position: 'sticky', top: 0, backgroundColor: 'var(--background)' }}>
                    <tr style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: '600', color: 'var(--text-muted)', fontSize: '0.75rem' }}>PDF</th>
                      <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: '600', color: 'var(--text-muted)', fontSize: '0.75rem' }}>Profile</th>
                      <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: '600', color: 'var(--text-muted)', fontSize: '0.75rem' }}>Citation</th>
                      <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: '600', color: 'var(--text-muted)', fontSize: '0.75rem' }}>Image</th>
                    </tr>
                  </thead>
                  <tbody>
                    {syncResult.syncedClients.map((client, i) => (
                      <tr key={i} style={{ borderBottom: i < syncResult.syncedClients.length - 1 ? '1px solid var(--border)' : 'none' }}>
                        ...
                        <td style={{ padding: '0.75rem', textAlign: 'center', color: 'var(--success)', fontWeight: '600' }}>{client.pdf}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', color: 'var(--success)', fontWeight: '600' }}>{client.profile}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', color: 'var(--success)', fontWeight: '600' }}>{client.citation}</td>
                        <td style={{ padding: '0.75rem', textAlign: 'center', color: 'var(--success)', fontWeight: '600' }}>{client.image}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
