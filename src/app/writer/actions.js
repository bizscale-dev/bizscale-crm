...
export async function logWebPost(prevState, formData) {
  const db = await getDb();
  const session = await verifySession();
  if (!session) return { error: 'Not authenticated' };
  ...
  const task = await db.prepare('SELECT * FROM web_writing_tasks WHERE id = ? AND writer_id = ?').get(taskId, session.userId);
  if (!task) return { error: 'Task not found or not authorized' };

  try {
    await db.prepare('INSERT INTO web_writing_logs (task_id, title, url, word_count, notes, logged_by) VALUES (?, ?, ?, ?, ?, ?)')
      .run(taskId, title, url, word_count, notes, session.userId);

    const logCount = (await db.prepare('SELECT COUNT(*) as c FROM web_writing_logs WHERE task_id = ?').get(taskId)).c;
    await db.prepare('UPDATE web_writing_tasks SET completed_count = ? WHERE id = ?').run(logCount, taskId);
...
export async function deleteWebPost(logId) {
  const db = await getDb();
  const session = await verifySession();
  if (!session) return { error: 'Not authenticated' };

  const log = await db.prepare(`
    SELECT wl.*, wt.writer_id FROM web_writing_logs wl
    JOIN web_writing_tasks wt ON wt.id = wl.task_id
    WHERE wl.id = ?
  `).get(logId);
  ...
  try {
    await db.prepare('DELETE FROM web_writing_logs WHERE id = ?').run(logId);
    const logCount = (await db.prepare('SELECT COUNT(*) as c FROM web_writing_logs WHERE task_id = ?').get(log.task_id)).c;
    await db.prepare('UPDATE web_writing_tasks SET completed_count = ? WHERE id = ?').run(logCount, log.task_id);
